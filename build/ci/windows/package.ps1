...
$UUIDGEN = "C:\Program Files (x86)\Windows Kits\10\bin\x64\uuidgen.exe"
$WIX_DIR = $env:WIX

switch ($PACKAGE_TYPE) {
    "portable" { goto :PACK_PORTABLE }
    "7z" { goto :PACK_7z }
    "msi" { goto :PACK_MSI }
    "dir" { goto :PACK_DIR }
    default {
...
if ($DO_SIGN -eq "ON") {
    cmd /c "$SIGN --secret $SIGN_CERTIFICATE_ENCRYPT_SECRET --pass $SIGN_CERTIFICATE_PASSWORD --dir $INSTALL_DIR" || exit 1
} else {
...
(Get-Content build/Packaging.cmake) -replace '00000000-0000-0000-0000-000000000000', $PACKAGE_UUID | Set-Content build/Packaging.cmake
(Get-Content build/Packaging.cmake) -replace '11111111-1111-1111-1111-111111111111', $UPGRADE_UUID | Set-Content build/Packaging.cmake
...
cmd /c "msvc_build.bat package $TARGET_PROCESSOR_BITS"
...
$FILEPATH = Get-ChildItem -Path "$BUILD_DIR" -Filter "*.msi" -Recurse | Select-Object -First 1
...
if ($DO_SIGN -eq "ON") {
    cmd /c "$SIGN --secret $SIGN_CERTIFICATE_ENCRYPT_SECRET --pass $SIGN_CERTIFICATE_PASSWORD --name $ARTIFACT_NAME --file $ARTIFACT_PATH" || exit 1
}
